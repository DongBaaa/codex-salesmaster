...
function Resolve-DotnetCommand {
    ...
    throw "sync 복구 점검용 dotnet 실행 파일을 찾지 못했습니다. ProjectRoot=$ProjectRoot"
}

$dotnet = Resolve-DotnetCommand -ProjectRoot $ProjectRoot
$projectPath = Join-Path $ProjectRoot "tasks\SyncRecoveryVerifier\SyncRecoveryVerifier.csproj"
if (-not (Test-Path -LiteralPath $projectPath)) {
    throw "SyncRecoveryVerifier 프로젝트를 찾을 수 없습니다: $projectPath"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "sync 복구 자동 점검 실행이 실패했습니다. exit=$LASTEXITCODE"
}

Write-Host "sync 복구 JSON 리포트: $OutputPath"
Write-Host "sync 복구 Markdown 리포트: $MarkdownOutputPath"

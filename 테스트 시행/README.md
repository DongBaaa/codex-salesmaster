...
- 위 항목을 한 번에 묶어 납품 리포트로 남길 때는 `D:\거래플랜\tools\verification\Invoke-GeoraePlanPaidDeliveryGate.ps1 -Strict -LocalCacheAppDataRoot "<사용자 AppData 루트>" -AndroidApkPath "<새 APK>"`를 사용합니다. `-Strict`에서는 하위 경고도 실패로 전파되어 경고가 상위 PASS로 숨지 않습니다.

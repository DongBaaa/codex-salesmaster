...
거래플랜 수정은 앞으로 이 폴더 기준으로 **내 PC 로컬 테스트 서버 선검증 → 이상 없으면 NAS 반영 → Git 반영** 순서로 진행합니다.
...
7. 수정사항 확인 완료 후 `D:\거래플랜\테스트 시행\검증완료-반영.cmd` 또는 `D:\거래플랜\테스트 시행\검증완료-반영.ps1` 로 NAS/Git 반영
...
- 실행 시 체크리스트가 통과된 경우에만 NAS/Git 반영을 진행합니다.

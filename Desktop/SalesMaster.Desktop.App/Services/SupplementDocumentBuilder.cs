...
public static class SupplementDocumentBuilder
{
    ...
    private const double SheetWidth = 730;
    private const double EstimateSheetHeight = 980;
    private const double ClaimSheetHeight = 980;
    private const double LineThickness = 0.8;

    private static readonly Brush BorderBrushBlack = Brushes.Black;
    ...
    [
        40, 18, 18, 18, 18, 18, 25, 15, 20.1, 20.1, 15, 20.1,
        18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18,
        20.1, 20.1, 15, 20.1, 18, 18, 18, 18, 15, 20.1
    ];
    ...
    [
        40, 20.1, 20.1, 20.1, 20.1, 20.1, 25, 15, 25, 18, 18,
        50.1, 18, 18, 30, 30, 18, 18, 18, 18, 20.1, 20.1, 20.1,
        20.1, 20.1, 20.1, 20.1, 20.1, 20.1, 15, 20.1
    ];
    ...
    public static FixedDocument BuildEstimateDocument(LocalInvoice invoice, LocalCustomer customer, LocalCompanyProfile company)
    {
        ...
        AddMergedCell(grid, 1, 1, 1, 19, "견     적     서",
            bold: true, fontSize: 42, align: TextAlignment.Center, foreground: TitleBrush, borderBrush: Brushes.Transparent);

        AddMergedCell(grid, 2, 1, 2, 8, string.Empty);
        AddMergedCell(grid, 3, 1, 3, 8, string.Empty);
        AddMergedCell(grid, 4, 1, 4, 8, $"{receiver} 귀하", bold: true, fontSize: 15);
        AddMergedCell(grid, 5, 1, 5, 8, string.Empty);
        AddMergedCell(grid, 6, 1, 6, 8, string.Empty);

        AddMergedCell(grid, 2, 9, 6, 9, "공\n\n급\n\n자", bold: true, align: TextAlignment.Center, fontSize: 14);

        AddMergedCell(grid, 2, 10, 2, 11, "등록번호", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 2, 12, 2, 19, SpaceBusinessNumber(company.BusinessNumber), fontSize: 15);

        AddMergedCell(grid, 3, 10, 3, 11, "상호명", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 3, 12, 3, 14, Safe(company.TradeName, "기본 상호"));
        AddMergedCell(grid, 3, 15, 3, 16, "대   표", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 3, 17, 3, 19, Safe(company.Representative, "대표자"));

        AddMergedCell(grid, 4, 10, 4, 11, "주   소", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 4, 12, 4, 19, Safe(company.Address));

        AddMergedCell(grid, 5, 10, 5, 11, "업   태", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 5, 12, 5, 14, Safe(company.BusinessType));
        AddMergedCell(grid, 5, 15, 5, 16, "종   목", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 5, 17, 5, 19, Safe(company.BusinessItem));

        AddMergedCell(grid, 6, 10, 6, 11, "연락처", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 6, 12, 6, 14, Safe(company.ContactNumber));
        AddMergedCell(grid, 6, 15, 6, 16, "팩   스", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 6, 17, 6, 19, Safe(customer.FaxNumber));

        AddMergedCell(grid, 7, 1, 7, 6, "견적금액 (공급가액+세액)", bold: true, align: TextAlignment.Center, fontSize: 13);
        AddMergedCell(grid, 7, 7, 7, 19, BuildAmountLabel(totalAmount), bold: true, fontSize: 26, foreground: TitleBrush);

        AddMergedCell(grid, 9, 1, 9, 3, "견적명", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 9, 4, 9, 19, Safe(invoice.Memo, "품목 견적"));
        AddMergedCell(grid, 10, 1, 10, 3, "견적유효기간", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 10, 4, 10, 19, $"{invoice.InvoiceDate:yyyy-MM-dd} ~ {invoice.InvoiceDate.AddDays(30):yyyy-MM-dd}");

        AddMergedCell(grid, 12, 1, 12, 1, "NO", bold: true, align: TextAlignment.Center, fontSize: 13);
        AddMergedCell(grid, 12, 2, 12, 5, "품명", bold: true, align: TextAlignment.Center, fontSize: 13);
        AddMergedCell(grid, 12, 6, 12, 9, "규격", bold: true, align: TextAlignment.Center, fontSize: 13);
        AddMergedCell(grid, 12, 10, 12, 10, "단위", bold: true, align: TextAlignment.Center, fontSize: 13);
        AddMergedCell(grid, 12, 11, 12, 11, "수량", bold: true, align: TextAlignment.Center, fontSize: 13);
        AddMergedCell(grid, 12, 12, 12, 13, "단가", bold: true, align: TextAlignment.Center, fontSize: 13);
        AddMergedCell(grid, 12, 14, 12, 16, "공급가액", bold: true, align: TextAlignment.Center, fontSize: 13);
        AddMergedCell(grid, 12, 17, 12, 19, "비고", bold: true, align: TextAlignment.Center, fontSize: 13);
        ...
        for (var i = 0; i < 13; i++)
        {
            ...
            AddMergedCell(grid, row, 1, row, 1, line is null ? string.Empty : (i + 1).ToString(CultureInfo.InvariantCulture), align: TextAlignment.Center);
            AddMergedCell(grid, row, 2, row, 5, line?.ItemNameOriginal ?? string.Empty);
            AddMergedCell(grid, row, 6, row, 9, line?.SpecificationOriginal ?? string.Empty);
            AddMergedCell(grid, row, 10, row, 10, line?.Unit ?? string.Empty, align: TextAlignment.Center);
            AddMergedCell(grid, row, 11, row, 11, line is null ? string.Empty : $"{line.Quantity:N0}", align: TextAlignment.Right);
            AddMergedCell(grid, row, 12, row, 13, line is null ? string.Empty : $"{line.UnitPrice:N0}", align: TextAlignment.Right);
            AddMergedCell(grid, row, 14, row, 16, line is null ? string.Empty : $"{line.LineAmount:N0}", align: TextAlignment.Right);
            AddMergedCell(grid, row, 17, row, 19, line?.Remark ?? string.Empty);
        }

        AddMergedCell(grid, 26, 1, 26, 11, "공  급  가  액  합  계", bold: true, align: TextAlignment.Center, fontSize: 13);
        AddMergedCell(grid, 26, 12, 26, 13, string.Empty);
        AddMergedCell(grid, 26, 14, 26, 16, $"{supplyAmount:N0}", bold: true, align: TextAlignment.Right, fontSize: 13);
        AddMergedCell(grid, 26, 17, 26, 19, string.Empty);

        AddMergedCell(grid, 27, 1, 27, 11, "부  가  세  액", bold: true, align: TextAlignment.Center, fontSize: 13);
        AddMergedCell(grid, 27, 12, 27, 13, string.Empty);
        AddMergedCell(grid, 27, 14, 27, 16, $"{vatAmount:N0}", bold: true, align: TextAlignment.Right, fontSize: 13);
        AddMergedCell(grid, 27, 17, 27, 19, ".");

        AddMergedCell(grid, 29, 1, 29, 3, "특이사항", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 29, 4, 29, 19, string.Empty);
        ...
        IReadOnlyList<string>? attachmentDisplayNames = null)
    {
        ...
        AddMergedCell(grid, 7, 1, 7, 6, "청구금액 (공급가액+세액)", bold: true, align: TextAlignment.Center, fontSize: 13);
        AddMergedCell(grid, 7, 7, 7, 19, BuildAmountLabel(totalAmount), bold: true, fontSize: 26, foreground: TitleBrush);

        AddMergedCell(grid, 9, 1, 9, 3, "용역명", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 9, 4, 9, 19, Safe(invoice.Memo, "납품 대금"));

        AddMergedCell(grid, 12, 1, 12, 19, "위 금액을 청구하오니 지급하여 주시기 바랍니다.", fontSize: 14);

        AddMergedCell(grid, 15, 1, 15, 3, "금융기관명", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 15, 4, 15, 7, "입금은행/계좌번호", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 15, 8, 15, 10, "예금주", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 15, 11, 15, 13, "공급가액", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 15, 14, 15, 16, "세액", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, 15, 17, 15, 19, "합계금액", bold: true, align: TextAlignment.Center);
        ...
        AddMergedCell(
            ...
            29,
            19,
            BuildAttachmentListText(attachmentDisplayNames),
            vertical: VerticalAlignment.Top,
            padding: new Thickness(10, 6, 10, 6));

        AddStampOverlay(grid, company.StampImage, 3, 18, 4, 19);
        AddFooterRow(grid, 31, company);
        return BuildCenteredSinglePageDocument(grid);
    }
    ...
    {
        AddMergedCell(grid, row, 1, row, 2, "담당자", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, row, 3, row, 5, Safe(company.Representative, "대표자"));
        AddMergedCell(grid, row, 6, row, 7, "연락처", bold: true, align: TextAlignment.Center);
        AddMergedCell(grid, row, 8, row, 10, Safe(company.ContactNumber));
        AddMergedCell(grid, row, 11, row, 13, "결재방법", bold: true, align: TextAlignment.Center);

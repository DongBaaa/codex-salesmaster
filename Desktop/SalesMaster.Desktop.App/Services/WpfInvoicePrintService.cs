...
public sealed class WpfInvoicePrintService : IPrintService
{
    ...
    private static readonly Brush HeaderFill = new SolidColorBrush(Color.FromRgb(244, 246, 250));
    private static readonly Brush BorderGray = new SolidColorBrush(Color.FromRgb(145, 156, 171));
        ...
        bool printWithPrice)
    {
        ...
        var isPurchase = invoice.VoucherType == VoucherType.Purchase;
        ...
        bool isFinalPage)
    {
        ...
        AddToGrid(root, BuildPurchaseLineTable(model, lines), 4);
        AddToGrid(root, BuildPurchaseTotals(model, isFinalPage), 5);

        var footer = CreateText("개발사: 희경 데이터베이스 1566-1186", 10);
        footer.HorizontalAlignment = HorizontalAlignment.Right;
        footer.Margin = new Thickness(0, 6, 0, 0);
        AddToGrid(root, footer, 6);
        ...
        string representative)
    {
        ...
        infoGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(78) });
        infoGrid.ColumnDefinitions.Add(new ColumnDefinition());
        for (var i = 0; i < 5; i++)
            infoGrid.RowDefinitions.Add(new RowDefinition { Height = new GridLength(25) });

        AddLabelValue(infoGrid, 0, "사업번호", businessNumber, Brushes.Black);
        AddLabelValue(infoGrid, 1, "상호", tradeName, Brushes.Black);
        AddLabelValue(infoGrid, 2, "전화번호", phone, Brushes.Black);
        AddLabelValue(infoGrid, 3, "주소", address, Brushes.Black, wrap: true);
        AddLabelValue(infoGrid, 4, "대표자", representative, Brushes.Black);
        ...
        IReadOnlyList<InvoicePrintLineModel> lines)
    {
        ...
        for (var row = 0; row < PurchaseRowsPerPage; row++)
            table.RowDefinitions.Add(new RowDefinition { Height = new GridLength(30) });

        AddTableCell(table, 0, 0, "No", Brushes.Black, isHeader: true, center: true);
        AddTableCell(table, 0, 1, "품명 / 규격", Brushes.Black, isHeader: true, center: true);
        AddTableCell(table, 0, 2, "단위", Brushes.Black, isHeader: true, center: true);
        AddTableCell(table, 0, 3, "수량", Brushes.Black, isHeader: true, center: true);
        AddTableCell(table, 0, 4, "단가", Brushes.Black, isHeader: true, center: true);
        AddTableCell(table, 0, 5, "금액", Brushes.Black, isHeader: true, center: true);
        ...
        for (var row = 0; row < PurchaseRowsPerPage; row++)
        {
            ...
            AddTableCell(table, row + 1, 0, line?.No.ToString() ?? string.Empty, Brushes.Black, center: true);
            AddTableCell(table, row + 1, 1, itemText, Brushes.Black);
            AddTableCell(table, row + 1, 2, line?.Unit ?? string.Empty, Brushes.Black, center: true);
            AddTableCell(table, row + 1, 3, line is null ? string.Empty : FormatQuantity(line.Quantity), Brushes.Black, alignRight: true);
            AddTableCell(table, row + 1, 4, line is null || !model.PrintWithPrice ? string.Empty : FormatMoney(line.UnitPrice), Brushes.Black, alignRight: true);
            AddTableCell(table, row + 1, 5, line is null || !model.PrintWithPrice ? string.Empty : FormatMoney(line.Amount), Brushes.Black, alignRight: true);
        }
    ...
    private static Border BuildPurchaseTotals(InvoicePrintModel model, bool isFinalPage)
    {
        ...
        AddTotalsPair(grid, 0, 0, "전표메모", isFinalPage ? model.Memo : string.Empty, Brushes.Black, false, false);
        AddTotalsPair(grid, 0, 2, "공급가", isFinalPage ? FormatMoney(model.SupplyAmount) : string.Empty, Brushes.Black, true, true);
        AddTotalsPair(grid, 0, 4, "부가세", isFinalPage ? FormatMoney(model.VatAmount) : string.Empty, Brushes.Black, true, true);
        AddTotalsPair(grid, 0, 6, "합계", isFinalPage ? FormatMoney(model.TotalAmount) : string.Empty, Brushes.Black, true, true);

        AddTotalsPair(grid, 1, 0, "전미수", isFinalPage ? "0" : string.Empty, Brushes.Black, false, true);
        AddTotalsPair(grid, 1, 2, "지불액", isFinalPage ? FormatMoney(model.PaidAmount) : string.Empty, Brushes.Black, false, true);
        AddTotalsPair(grid, 1, 4, "미수잔액", isFinalPage ? FormatSignedMoney(model.BalanceAmount) : string.Empty, Brushes.Black, false, true);
        AddTotalsPair(grid, 1, 6, string.Empty, isFinalPage ? string.Empty : "다음 페이지 계속", Brushes.Black, false, true);
        ...
        Brush accent)
    {
        ...
        left.Children.Add(CreateText("관리자", 10, null, accent));
        left.Children.Add(CreateText(string.IsNullOrWhiteSpace(model.ManagerName) ? "-" : model.ManagerName, 11));
        left.Children.Add(CreateText("작성일자", 10, null, accent, marginTop: 2));
        ...
        Brush accent)
    {
        ...
        for (var row = 0; row < SalesRowsPerPage; row++)
        {
            ...
            AddTableCell(table, row + 1, 1, line?.ItemName ?? (blankHint ? "*** 이하 여백 ***" : string.Empty), accent);
            AddTableCell(table, row + 1, 2, line?.Specification ?? string.Empty, accent);
        ...
        Brush accent)
    {
        ...
        var right = CreateText(
            $"수량합계 {FormatQuantity(quantitySum)}  * 개발회사 www.hkdb.co.kr  1566-1186",
            10,
            null,
            accent);
        right.HorizontalAlignment = HorizontalAlignment.Right;
        ...
        bool wrap = false)
    {
        var labelBorder = new Border
        {
            ...
            Child = CreateText(label, 10.5, FontWeights.Bold, borderBrush, TextAlignment.Center)
        };
        ...
        bool alignRight = false)
    {
        ...
        var textBlock = CreateText(
            ...
            isHeader ? borderBrush : Brushes.Black,
            alignment);

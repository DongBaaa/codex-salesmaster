...
public sealed partial class InventoryViewModel : ObservableObject
{
    ...
    [ObservableProperty] private decimal _uznetTotalQuantity;
    ...
    [ObservableProperty] private decimal _editUznetStock;
    ...
    public string SelectedOfficeDisplay => string.Equals(SelectedOfficeCode, DomainConstants.OfficeYeonsu, StringComparison.OrdinalIgnoreCase)
        ? "연수구 사무실"
        : "유즈넷";
    ...
    [RelayCommand(CanExecute = nameof(CanShowUznetOffice))]
    private void ShowUznetOffice()
    {
        SelectedOfficeCode = DomainConstants.OfficeUznet;
    }

    private bool CanShowUznetOffice()
        => IsAdmin && !string.Equals(SelectedOfficeCode, DomainConstants.OfficeUznet, StringComparison.OrdinalIgnoreCase);
    ...
    partial void OnSelectedOfficeCodeChanged(string value)
    {
        ...
        ShowUznetOfficeCommand.NotifyCanExecuteChanged();
        ShowYeonsuOfficeCommand.NotifyCanExecuteChanged();
        ApplyFilter();
    }

    partial void OnUznetTotalQuantityChanged(decimal value) => OnPropertyChanged(nameof(UznetTabText));
    ...
    private async Task LoadInventoryStateAsync()
    {
        ...
        if (stocks.Count == 0)
        {
            foreach (var item in _allItems.Where(item => item.CurrentStock != 0m))
            {
                ...
                {
                    [DomainConstants.OfficeUznet] = item.CurrentStock
                };
                UznetTotalQuantity += item.CurrentStock;
            }
        ...
        foreach (var stock in stocks)
        {
            ...
            if (string.Equals(officeCode, DomainConstants.OfficeYeonsu, StringComparison.OrdinalIgnoreCase))
                ...
                UznetTotalQuantity += stock.Quantity;
        }
    }
    ...
    private InventoryItemRow BuildRow(LocalItem item)
        => new(
            item,
            GetOfficeQuantity(item.Id, DomainConstants.OfficeUznet),
            GetOfficeQuantity(item.Id, DomainConstants.OfficeYeonsu),
            SelectedOfficeCode);
    ...
    private void LoadFormFromItem(InventoryItemRow row)
    {
        ...
        EditUznetStock = row.UznetQuantity;
    ...
    private string ResolveOfficeCodeFromWarehouse(string? warehouseCode)
    {
        ...
        return string.Equals(normalizedWarehouseCode, DomainConstants.WarehouseYeonsuMain, StringComparison.OrdinalIgnoreCase)
    ...
    private string ResolveWarehouseDisplayName(string? warehouseCode)
    {
        ...
        if (string.Equals(normalizedWarehouseCode, DomainConstants.WarehouseUznetMain, StringComparison.OrdinalIgnoreCase))
            return "유즈넷 창고";
    ...
    private static string ResolveDefaultOfficeCode(SessionState session)
        => string.Equals(session.OfficeCode, DomainConstants.OfficeYeonsu, StringComparison.OrdinalIgnoreCase)
            ? DomainConstants.OfficeYeonsu
            : DomainConstants.OfficeUznet;

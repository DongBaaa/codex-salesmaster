...
public static class DiagnosticUserMessageFormatter
{
    ...
    public static string DescribeOutboxError(string? errorMessage, string? status = "Prepared")
    {
        var normalizedStatus = status?.Trim().ToUpperInvariant();
        if (normalizedStatus == "ACKNOWLEDGED")
            return "서버에서 전송 완료를 확인했습니다.";

        if (string.IsNullOrWhiteSpace(errorMessage))
            return normalizedStatus switch
            {
                "PREPARED" => "서버 전송 대기 중입니다.",
                "SENT" => "전송 후 서버의 완료 확인을 기다리고 있습니다.",
                "FAILED" => "서버 전송에 실패했습니다. 진단 리포트에서 상세 내용을 확인하세요.",
                _ => "전송 상태를 확인하세요."
            };

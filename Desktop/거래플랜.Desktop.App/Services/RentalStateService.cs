...
WHERE ""AssignedUsername"" <> '';", ct);
    ...
    private static void ApplyRentalBillingMethodReceiptAmount(LocalTransaction transaction, string? billingMethod, decimal amount)
    {
        ...
        transaction.ReceiptTotal = normalizedAmount;
        transaction.SettlementAmount = normalizedAmount;

        switch (NormalizeBillingMethod(billingMethod))
        {
            case "현금":
                transaction.CashReceipt = normalizedAmount;
                break;
            case "카드":
                transaction.CardReceipt = normalizedAmount;
                break;
            default:
                transaction.BankReceipt = normalizedAmount;
                break;
        }
    }
    ...
    private static bool CanStartRentalBillingInvoice(SessionState? session)
        => session is not null &&
           (session.HasAdministrativePrivileges || session.HasPermission(AppPermissionNames.InvoiceEdit));
        ...
        string? fallbackItemName = null)
    {
        ...
        var representativeName = FirstNonEmpty(
            ...
            RentalCatalogValueNormalizer.NormalizeItemNameDisplayName(fallbackItemName ?? string.Empty),
            "대표 장비");
        var manufacturerRepresentativeName = FirstNonEmpty(
        ...
        return IsAutoGeneratedBundleSpecification(explicitSpecification, representativeName) ||
               IsAutoGeneratedBundleSpecification(explicitSpecification, manufacturerRepresentativeName);
    }

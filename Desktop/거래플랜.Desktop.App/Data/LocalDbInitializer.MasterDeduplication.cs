...
public static partial class LocalDbInitializer
{
    ...
    private static Task MergeDuplicateItemsAsync(LocalDbContext db)
    {
        ArgumentNullException.ThrowIfNull(db);

        // 품목 병합은 서버 동기화 tombstone, 대표 품목 선택, 권한/참조 검증이 필요한 명시적 작업이다.
        // 시작 초기화에서 hard-delete하면 다른 PC/서버에 삭제 사실이 전파되지 않으므로 자동 병합하지 않는다.

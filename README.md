...
## 현재 버전 상태
### 공개 stable
- `[공개]` Windows PC `1.1.699`
- `[공개]` Android 표시 버전은 `0.2.82`입니다. APK 내부 versionCode는 공개 매니페스트에 없으므로 공개값으로 단정하지 않습니다.
- `[공개]` 2026-08-28 확인에서 `trade.2884.kr/healthz`와 stable manifest는 HTTP 200·redirect 0이며 `fileDeletionLeaseProtocol=shared-flock-v1`입니다.
...
### 현재 소스·테스트판
- `[로컬검증]` Windows PC 현재 소스 `1.1.701`, FileVersion `1.1.701.0`이며 공개 stable `1.1.699`보다 높은 live 반영 후보입니다.
...
- 현재 소스 `1.1.699`의 정식 패키지 생성과 Linux PC live 반영을 완료했습니다. 현재 공개 stable ZIP SHA-256은 `961F99BAFD8F8F66084CF74E7173171D7A627CE896DDF90385F27741182690C6`입니다.

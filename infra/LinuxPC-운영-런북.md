...
> 유료 납품 증거에 실제 사용자 PC 로컬 캐시 동기화까지 포함하려면 `-LocalCacheAppDataRoot "<사용자 AppData 루트>" -RequireLocalCacheConsistencyCheck`를 같이 지정합니다. 이때 로컬 캐시 검증이 skip되면 배포 게이트는 실패해야 합니다.

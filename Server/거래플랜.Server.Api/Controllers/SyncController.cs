...
public sealed class SyncController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var dto in payload)
        {
            ...
            if (!dto.IsDeleted &&
                await ShouldProtectExistingInvoiceFromSameIdStructuralMutationAsync(entity, dto, cancellationToken) &&
                HasSameIdInvoiceStructuralMutation(entity, dto))
            {
                AddClientConflict(
                    dto,
                    nameof(Invoice),
    ...
    private async Task<bool> ShouldProtectExistingInvoiceFromSameIdStructuralMutationAsync(
        Invoice existing,
        InvoiceDto dto,
        CancellationToken cancellationToken)
    {
        if (HasGuid(existing.LinkedRentalBillingProfileId) ||
        ...
        if (existing.Payments.Any(payment => !payment.IsDeleted))
            return true;
        ...
        if (versionGroupId != Guid.Empty &&
            await _dbContext.Invoices.IgnoreQueryFilters()
                .AsNoTracking()
                .AnyAsync(invoice =>
        ...
        return await HasActivePaymentSideEffectsForInvoiceDeleteAsync([existing.Id], cancellationToken);
    }

    private static bool HasSameIdInvoiceStructuralMutation(Invoice existing, InvoiceDto dto)
    {

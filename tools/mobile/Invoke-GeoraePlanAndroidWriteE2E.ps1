...
function Set-LoginTextField {
    ...
    for ($attempt = 1; $attempt -le 3; $attempt++) {
        ...
        $beforeDump = Get-UiDump -AdbPath $AdbPath -DeviceId $DeviceId -EvidenceDirectory $EvidenceDirectory -Name "mobile-write-e2e-$Timestamp-login-$safeFieldName-before$attempt"
        ...
        Tap-Point -AdbPath $AdbPath -DeviceId $DeviceId -X $fieldNode.Point.X -Y $fieldNode.Point.Y
        Start-Sleep -Milliseconds 700
        $focusDump = Get-UiDump -AdbPath $AdbPath -DeviceId $DeviceId -EvidenceDirectory $EvidenceDirectory -Name "mobile-write-e2e-$Timestamp-login-$safeFieldName-focus$attempt"
...
try {
    ...
    if ($dump.Content.Contains('계정 로그인') -or ($dump.Content.Contains('로그인') -and $dump.Content.Contains('비밀번호'))) {
        ...
        $dump = Set-LoginTextField -AdbPath $resolvedAdb -DeviceId $deviceId -EvidenceDirectory $EvidenceDirectory -Timestamp $timestamp -FieldName '아이디' -IsPassword $false -Value $Username -VerifyPlainText
        ...
        $dump = Set-LoginTextField -AdbPath $resolvedAdb -DeviceId $deviceId -EvidenceDirectory $EvidenceDirectory -Timestamp $timestamp -FieldName '비밀번호' -IsPassword $true -Value $Password
        Invoke-Adb -AdbPath $resolvedAdb -Arguments @('-s', $deviceId, 'shell', 'input', 'keyevent', 'KEYCODE_ESCAPE') | Out-Null
        Start-Sleep -Seconds 1

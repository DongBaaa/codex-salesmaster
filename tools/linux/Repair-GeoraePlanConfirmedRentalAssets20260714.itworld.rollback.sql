...
BEGIN
    ...
    WHERE "AssetId" IN (
        ...
        UNION ALL SELECT 'd0faac89-935a-4988-a7bd-67aee7c8ccf5'::uuid
    );

    DELETE FROM "RentalAssets" WHERE "Id" = 'd0faac89-935a-4988-a7bd-67aee7c8ccf5';
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'georaeplan_itworld rental asset rollback verification failed';
    END IF;

    IF EXISTS (SELECT 1 FROM "RentalAssets" WHERE "Id" = 'd0faac89-935a-4988-a7bd-67aee7c8ccf5') THEN
        RAISE EXCEPTION 'inserted MR asset remains after rollback';
    END IF;
END $$;

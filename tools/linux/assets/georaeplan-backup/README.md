...
두 dump는 `pg_restore -l`, 두 tar는 `tar -tzf`, 전체 데이터 파일은 `sha256sum -c`를 통과해야 한다. 또한 PostgreSQL 16의 cluster-wide `pg_current_snapshot()` 토큰을 첫 dump 직전과 두 번째 dump 직후에 읽어 값이 완전히 같을 때만 두 논리 dump를 같은 무변경 MVCC 구간의 결과로 인정한다. 중간에 쓰기 transaction 가시성이 바뀌면 staging을 폐기하고 마지막 정상 상태를 보존한다. 검증된 세트를 원자 게시하고 `ops/state/backup-status.txt`가 새 세트를 가리키도록 `backup=ok`로 원자 교체한 뒤에만 만료 세트를 정리한다. 따라서 상태 교체 실패가 마지막 성공 상태가 참조하는 기존 세트를 먼저 지우지 않는다. 실패는 성공 상태를 덮지 않고 `backup-failure-status.txt`와 실행별 로그에 별도로 남는다.

...
이 자산은 `/srv/georaeplan` 운영 compose의 `postgres` 서비스만 읽어 중앙 DB와 모든 업체 DB를 custom dump로 만들고, 파일 저장소와 Data Protection key ring을 함께 보관한다. 컨테이너·Docker·PostgreSQL·API를 시작하거나 재시작하지 않는다.

## 완성 백업 세트

백업은 `/srv/georaeplan/backups/automatic/.staging`에서 생성·검증한 뒤 같은 파일시스템의 `sets/backup_*.complete` 디렉터리로 한 번에 이동한다.

- `databases.txt` (`DB명<TAB>dump 파일명<TAB>7개 핵심 테이블 건수 SHA-256`)
- `georaeplan.dump`, `georaeplan_itworld.dump`
- 존재할 경우 `georaeplan_usenet.dump`, `georaeplan_org_<계정>.dump`
- `files.tar.gz`
...
모든 dump는 `pg_restore -l`, 두 tar는 `tar -tzf`, 전체 데이터 파일은 `sha256sum -c`를 통과해야 한다. PostgreSQL의 cluster-wide `pg_current_snapshot()` 토큰을 첫 dump 직전과 마지막 dump 직후에 읽어 transaction 가시성 변화가 감지되면 실패한다. 이 토큰 비교가 여러 `pg_dump`의 단일 exported snapshot을 뜻하지는 않으므로, manifest의 모든 DB에서 `Users`, `Customers`, `Items`, `Transactions`, `RentalAssets`, `Invoices`, `Payments` 건수 해시를 해당 dump 직전과 직후에 계산해 같은 경우에만 게시한다. DB별 기대 해시는 `databases.txt`에 기록되고 이 파일 자체도 `metadata.txt` 및 `SHA256SUMS`에 결박된다. 이후 외부 replica 복구 드릴은 manifest의 모든 DB를 exact 비교한다.
...
- 중앙 `georaeplan`, 필수 `georaeplan_itworld`, 선택 `georaeplan_usenet`, 동적 `georaeplan_org_[a-z0-9_]+`를 PostgreSQL catalog에서 자동 발견한다. 필수 DB 누락, 중복, 허용 이름 밖의 항목, API 연결 문자열과의 불일치는 실패한다.
- 발견한 모든 DB를 순차 dump하는 동안 cluster snapshot 토큰이 달라지면 `database_snapshot_drift`로 실패한다. 이 검사는 서비스를 멈추거나 쓰기를 차단하지 않고, 일관성을 확인할 수 없는 실행을 complete set으로 게시하지 않는 실패 폐쇄 경계다.

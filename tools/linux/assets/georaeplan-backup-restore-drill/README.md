...
이 도구는 승인된 외부 replica의 현재 두 PostgreSQL custom dump를 운영 DB와 완전히 분리된 일회성 PostgreSQL 컨테이너에 복원한다. 운영 API·PostgreSQL·nginx·거래플랜 compose service를 시작·중지·재시작하거나 수정하지 않는다.
...
- `georaeplan.dump`와 `georaeplan_itworld.dump`를 각각 빈 DB에 `pg_restore --exit-on-error --no-owner --no-privileges`로 복원한다.
- 두 DB에서 `__EFMigrationsHistory`, `Users`, `Items`, `Transactions`, `RentalAssets`, `Invoices`, `Payments`를 실제 조회한다. 복원 건수 SHA-256은 `SHA256SUMS`에 결박된 `metadata.txt`의 백업 시점 기대값과 각각 exact 일치해야 하며, 일치한 경우에만 원문 건수 없이 SHA-256과 `business_count_digest_contract=source_metadata_match`를 status에 기록한다.
...
현재 환경에서는 외부 mount가 없으므로 실제 상태는 계속 `restore_drill=not_proven`이다.

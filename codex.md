...
- Windows 검증과 ZIP·EXE·MSI 생성이 통과하면 같은 작업에서 동일 버전의 live stable 매니페스트까지 게시한다. 설치본만 앞선 상태로 작업을 완료하지 않는다.
- 새 데스크톱 ZIP·EXE·MSI 생성과 live 게시 검증이 모두 통과한 뒤에는 최신 버전 설치본만 유지하고 이전 버전 설치본 파일은 삭제한다. 실패 중에는 기존 설치본을 삭제하지 않으며, 롤백·감사용 매니페스트와 배포 증거는 보존한다.
- 데스크톱 배포 완료 전에 공개 stable 매니페스트의 버전·생성 ID, ZIP·EXE·MSI 해시·크기, 다운로드 응답을 검증하고 `현재 설치 버전 > 배포 대상 버전` 상태가 아닌지 확인한다.
